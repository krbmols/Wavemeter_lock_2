...
import csv
import numpy as np
from PyQt5.QtWidgets import QApplication, QMainWindow, QGridLayout, QWidget,QCheckBox, QLabel, QVBoxLayout, QHBoxLayout
...
class Channel(QWidget):  # a class for the widgets belonging to a particular channel
    ...
    def __init__(self, i, parent=None):
        ...
        self.data = [0, 0, 0,
                     0]  # initial values for the channel's stored data. In order of frequency, saturation, status, and time.
        ...
        self.labelPP.setAlignment(Qt.AlignRight | Qt.AlignBottom)
        self.labelPPLT = QLabel()
        self.labelPPLT.setFont(QFont("Arial", 30))
        self.labelPPLT.setAlignment(Qt.AlignRight | Qt.AlignTop)
        ...
        channelLayout.addWidget(self.labelPPLT, 3, 1, 1, 1)
        ...
        channelLayout.addWidget(self.graphLT, 0, 4, 4, 1)
        channelLayout.setSpacing(0)
        channelLayout.setContentsMargins(0, 0, 0, 0)

    def updateLabel(self, name1, name2, name3, name4, name5,
                    name6):  # updates the value in the labels. Called upon by updateChannel
        self.label1.setText("%s" % name1)
        self.label2.setText("%s" % name2)
        self.labelPP.setText("%s" % name3)
        self.labelPPLT.setText("%s" % name6)
    ...
    def updateGraph(self):  # redraws the plot's 'line', aka datapoints. Called upon by updateChannel
        ...
        dy = [(value, '{:.3f}'.format(value)) for value in np.linspace(round(min(self.f), 3), round(max(self.f), 3),
                                                                       5)]  # creates the values to go on the left y-axis.
        ay.setTicks([dy,
                     []])  # pyqtgraph's automatic y-axis labels don't show decimal places, so we put in our own labels given by dy.
        self.graph.setYRange(1.5 * min(self.f) - 0.5 * np.mean(self.f) - .001, 1.5 * max(self.f) - 0.5 * np.mean(
            self.f) + .001)  # we set the graph range to automatically fit the channel's data. The extra 1 MHz (0.001) space makes the data easier to view.
    ...
    def updateGraphLT(self):  # redraws the plot's 'line', aka datapoints. Called upon by updateChannel
        ...
        ayLT.setTicks([dyLT,
                       []])  # pyqtgraph's automatic y-axis labels don't show decimal places, so we put in our own labels given by dy.
        self.graphLT.setYRange(1.5 * min(self.fLT) - 0.5 * np.mean(self.fLT) - .001,
                               1.5 * max(self.fLT) - 0.5 * np.mean(
                                   self.fLT) + .001)  # we set the graph range to automatically fit the channel's data. The extra 1 MHz (0.001) space makes the data easier to view.
    ...
    def updateChannel(self):
        ...
        if self.stdLT <= 1:
            name6 = '±' + '{:.1f}'.format(1000 * self.stdLT) + " MHz"
        elif self.stdLT <= 1E3:
            name6 = '±' + '{:.2f}'.format(self.stdLT) + " GHz"
        elif self.stdLT <= 1E6:
            name6 = '±' + '{:.2f}'.format(0.001 * self.stdLT) + " THz"
        else:
            name6 = ""
        self.updateLabel(f"{num}", (f"{decimal}" + "000")[:4], name3,
                         '{:.1f}'.format(self.light / self.f[-1]) + " nm", '{:.2f}'.format(self.data[1]),
                         name6)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.targets = [353600, 351721, 508849, 508333, 325100, 472150, 451300, 713289, 328967, 295157, 282825.8,
                        285228]
        self.activeTargets=[]
        self.targetsLength=len(self.targets)
        self.threshold = 200
        self.frequency = 0
        self.storedData = [[0, 0]] * self.targetsLength
        self.timeStart = time()
        self.fileName = 'wm8Data30.csv'
        ...
        self.targetWidgets = [0] * self.targetsLength;
        self.mainLayout = QVBoxLayout()
        self.mainWidget = QWidget()
        self.mainLayout.addLayout(self.targetLayout)
        self.mainWidget.setLayout(self.mainLayout)
        self.setCentralWidget(self.mainWidget)
        self.channels = [0] * self.targetsLength
        self.mainLayout.setSpacing(0)
        self.mainLayout.setContentsMargins(0, 0, 0, 0)

    # def removeTarget(self,s):
    # print(self.targetWidgets[].index)

    def createData(self):
        self.wavelength, self.saturation, self.status, self.timeStep = device.get_measurement()
        self.timeStep=time()-self.timeStart

        self.criteria=self.wavelength>=300 and self.wavelength<=1200 and self.saturation>=0.03 and self.saturation<=1
        if self.criteria:
            self.frequency=self.light/self.wavelength
            self.freqQueued=True
            self.fCalibration=self.NaD2
            proximity = []
            for i in range(self.targetsLength):
                proximity.append(abs(self.frequency-self.targets[i-1]))
            if min(proximity)<=self.threshold:
                self.targetMatchIndex=proximity.index(min(proximity))
                self.targetMatch=self.targets[self.targetMatchIndex]
                if abs(self.targetMatch-self.fCalibration)<=self.threshold:
                    self.calibrator=True

                    if len(self.wmError)>=self.wmAveraging:
                        self.wmError=self.wmError[1:]
                    self.wmError.append(self.frequency-self.fCalibrator)
                prop=self.frequency/self.fCalibration
                self.frequency-=np.mean(self.wmError)*prop
                self.data=[self.frequency,self.saturation,self.status,self.timeStep]
                self.storedData[self.targetMatchIndex]=self.data[:2]
                if self.channels[self.targetMatchIndex]==0:
                    self.channels[self.targetMatchIndex]=Channel(self.targetMatchIndex)
                    self.mainLayout.addWidget(self.channels[self.targetMatchIndex])
                else:
    ...
    def updateWidgets(self):
        ...
        for i in range(len(self.targets)):
            if self.channels[i] != 0:
                if self.timeStep - self.channels[i].data[-1] >= 1:
