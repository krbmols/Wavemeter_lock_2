...
class Channel(QWidget): #a class for the widgets belonging to a particular channel
    ...
    def updateLabel(self, name1,name2,name3,name4,name5,name6): #updates the value in the labels. Called upon by updateChannel
    ...
    def updateGraphLT(self): #redraws the plot's 'line', aka datapoints. Called upon by updateChannel
        ...
        dyLT = [(value, '{:.3f}'.format(value)) for value in np.linspace(round(min(self.fLT),3),round(max(self.fLT),3),5)] #creates the values to go on the left y-axis.
        ayLT.setTicks([dyLT, []]) #pyqtgraph's automatic y-axis labels don't show decimal places, so we put in our own labels given by dy.
        self.graphLT.setYRange(1.5*min(self.fLT)-0.5*np.mean(self.fLT)-.001,1.5*max(self.fLT)-0.5*np.mean(self.fLT)+.001) #we set the graph range to automatically fit the channel's data. The extra 1 MHz (0.001) space makes the data easier to view.
    ...
    def updateChannel(self):
        ...
        if self.stdLT<=1:
            ...
            name6=""
        self.updateLabel(f"{num}",(f"{decimal}"+"000")[:4],name3,'{:.1f}'.format(self.speed_of_light/self.f[-1])+" nm",'{:.2f}'.format(self.data[1]),name6)
...
#        self.setFixedSize(windowSize)
        ...
        self.targets=[353600,351721,508849,508333,325100,472137,451300,713289,328967,295157,352113.051914494,282825.8,285228]
        self.threshold=200
        ...
        for i in range(len(self.targets)):
            self.targetWidgets[i] = QCheckBox(str(self.targets[i]),self)
            self.targetWidgets[i].setChecked(False)
            self.targetWidgets[i].index=i
            # self.targetWidgets[i].stateChanged.connect(self.removeTarget)
            # self.label1.setFont(QFont("Arial",20))
            
            # self.label1.setAlignment(Qt.AlignTop)
            # self.label1.setText("%s" % self.targets[i])
        ...
        for i in range(len(self.targets)):
            if self.channels[i]!=0:
                if self.time_step-self.channels[i].data[-1]>=1:
                    ...
                    self.adjustSize()
                    self.targetWidget.setChecked(False)
