...
# 			self.serial_port.close()
	...
	def get_measurement(self):
		...
		Returns:
			...
			values is explained in the user's manual.

		"""
		# Discard data until we see the marker that indicates the start of a
		# measurement
		while self.serial_port.read(1)[0] != BristolRS422.START_TOKEN:
			pass
		...
		while len(bytes_received) < 20:
			...
			if escape_next:
				bytes_received.append(byte ^ BristolRS422.ESCAPE_XOR)
				escape_next = False
			elif byte == BristolRS422.ESCAPE_TOKEN:
				escape_next = True
			else:
