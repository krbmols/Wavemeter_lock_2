...
global writeFile
writeFile = open("N:\\\wavemeterEightChannelLogs\\20220223_fast_wm.csv", 'a', newline='')
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.listWidget.setMaximumSize(300, 300)
    ...
    def createData(self):
        ...
        if self.wavelength >= 300 and self.wavelength <= 2000 and self.saturation >= 0.04 and self.saturation <= 1:
            ...
            if abs(self.frequency - self.calibF) <= self.threshold:
            ...
            if self.validFreqNeedsAttention == True:
                ...
                self.rogueFreqs.append(f"t={dateTimeString}, f: {round(self.frequency,0)} GHz")
    ...
    def updateCalibrator(self):
        ...
        for i in range(8):
            ...
            if len(data)>0 and abs(data[0]-self.calibF)<=self.threshold:
                if abs(data[-1]-self.time_step)<=3:
                    ok=True
                    break
                else:
