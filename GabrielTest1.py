...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec  2 18:58:29 2021

@author: gabrielpatenotte
"""
import warnings
...
class Ui_MainWindow(object):
    ...
    def getDataset(self, csvfilename):
        ...
        """
        df = pd.read_csv(csvfilename, encoding='utf-8',skiprows=lambda x: (x != 0) and x % 10,usecols=['Timestamp','Frequency5', 'Frequency7','Frequency6','Frequency2'])

        df.replace(0, np.nan, inplace=True)
        LIST_OF_COLUMNS = df.columns.tolist()
        dataset = {}
        # time_format = '%Y-%m-%d %H:%M:%S.%f' # Please use this format for time_series-data_2.csv kind of time stamp
        time_format = '%Y-%m-%dT%H:%M:%S.%f'  # Please use this format for time_series-data_1.csv kind of time stamp

        for col in LIST_OF_COLUMNS:
            dataset[col] = df[col].iloc[0:].values
            try:
                dataset[col] = [datetime.strptime(str(i), time_format) for i in df[col].iloc[0:].values]
            except Exception as e:
                pass
                print(e)
